"""
adapted from https://github.com/ijmbarr/causalgraphicalmodels
"""
...
from scm.cgm import CausalGraphicalModel
...
def linear_model(parents, weights, offset=0, noise_scale=1):
    ...
    def model(**kwargs):
        ...
        a = np.array([kwargs[p] * w for p, w in zip(parents, weights)], dtype=np.float32)
        a = np.sum(a, axis=0)
