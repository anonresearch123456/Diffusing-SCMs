...
DATASET = [("diffusion_sex_binary_balanced",
                    ".../diffusion_data.h5",
                  ".../split.json"),]

DATASET_CTF = [("mediation_experiment_ctf",
               ".../cf_data.h5",
               ".../split.json")
               ]
...
def run_test(model_triplet: tuple, device: str = "cuda:0", batch_size: int = 32):
    ...
    data_module = UKBDataModule(dataset_path, split_path, batch_size=batch_size, num_workers=4,
                                crop_images=False, has_bin_age=False)
    ...
    data_module = UKBDataModule(dataset_path_ctf, split_path_ctf, batch_size=batch_size, num_workers=4,
                                crop_images=False, has_bin_age=False, has_orig_eid=True)
    data_module.setup("test")
    test_loader = data_module.test_dataloader()
    ...
    metrics["bacc"] = metrics.get("bacc", metrics.get(f"{model.target}/bacc", None))
    metrics["roc_auc"] = metrics.get("roc_auc", metrics.get(f"{model.target}/roc_auc", None))
...
def main():
    ...
    output_dir = Path("evaluation/mediation_experiment")
