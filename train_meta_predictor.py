...
from src.models.lightningmodules.predictmeta import MetaDataPrediction, MetadataPredictionAll
...
def adjust_checkpoint_callback(config: Optional[DictConfig]) -> None:
    ...
    if callbacks:
        if config.predictor.target in ["sex", "site"]:
            config.callbacks.model_checkpoint.monitor = "val/bacc"
            config.callbacks.model_checkpoint.mode = "max"
            config.callbacks.model_checkpoint.filename = "epoch-{epoch}-val_bacc-{val/bacc:.2f}"
...
def train(config: DictConfig):
    ...
    predictor_type = config.predictor.predictor_type
    if predictor_type in ["encoded", "reconstruct"]:
        LOG.info("Loading encoder from checkpoint")
        encoder: AutoencoderKL = AEModule.get_encoder_only_from_checkpoint(cfg_path=config.encoder.cfg_path,
                                                                           checkpoint_path=config.encoder.checkpoint_path)
    else:
        LOG.info("Doing pristine prediction on original data, no encoder needed")
        encoder = None

    if config.predictor.target != "all":
        LOG.info("Instantiating model <%s>", MetaDataPrediction)
        module: MetaDataPrediction = MetaDataPrediction(**config.predictor, encoder=encoder, class_weights=class_weights)
    else:
        LOG.info("Instantiating model <%s>", MetadataPredictionAll)
        module: MetadataPredictionAll = MetadataPredictionAll(**config.predictor, encoder=encoder, class_weights=class_weights)
...
@hydra.main(config_path="config", config_name="predict_meta.yaml", version_base="1.3")
